...
import { generatePDF, PDFLayout } from './services/pdfService';
...
const App: React.FC = () => {
  ...
  // Estados para o layout do PDF
  const [isLayoutModalOpen, setIsLayoutModalOpen] = useState(false);
  const [eventToPrint, setEventToPrint] = useState<EventData | null>(null);
  
  // Estado para exclusão
  const [eventToDelete, setEventToDelete] = useState<string | null>(null);
  
  // Estado para restauração
  const [pendingRestoreData, setPendingRestoreData] = useState<EventData[] | null>(null);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const updated = events.filter(ev => ev.id !== eventToDelete);
      setEvents(updated);
  ...
  const handlePrintClick = (event: EventData) => {
    setEventToPrint(event);
    setIsLayoutModalOpen(true);
  };

  const handleLayoutSelect = async (layout: PDFLayout) => {
    if (!eventToPrint) return;
    setIsLayoutModalOpen(false);
    setIsProcessing(true);
    try {
      await generatePDF(eventToPrint, layout);
    } catch (error) {
  ...
  const exportToJSON = () => {
    ...
    const link = document.createElement("a");
    link.setAttribute("href", url);
    link.setAttribute("download", `backup_rifas_${new Date().toISOString().slice(0,10)}.json`);
    link.click();
  };
  ...
  const confirmRestore = async () => {
    ...
    try {
      await db.clearAll();
      await db.importData(pendingRestoreData);
      await loadData();
      setPendingRestoreData(null);
      // Usando um estado temporário para mostrar sucesso sem alert() se possível, 
      // mas por enquanto manteremos o fluxo simples
  ...
  return (
    <div className="min-h-screen bg-[#0a0b14] text-slate-100 p-4 md:p-8">
      ...
      <input type="file" ref={jsonInputRef} onChange={handleRestoreJSON} accept=".json" className="hidden" />
      ...
      <header className="max-w-7xl mx-auto mb-10 flex flex-col md:flex-row justify-between items-center gap-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex gap-2">
            <button onClick={handleImportClick} title="Importar CSV" className="flex items-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 px-5 py-3 rounded-2xl font-bold text-sm transition-all active:scale-95"><Upload size={18} className="text-indigo-400" /><span className="hidden sm:inline">Importar CSV</span></button>
            <button onClick={() => jsonInputRef.current?.click()} title="Restaurar Backup JSON" className="flex items-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 px-5 py-3 rounded-2xl font-bold text-sm transition-all active:scale-95"><Database size={18} className="text-amber-400" /><span className="hidden sm:inline">Restaurar JSON</span></button>
            <button onClick={exportToJSON} title="Backup Completo JSON" className="flex items-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 px-5 py-3 rounded-2xl font-bold text-sm transition-all active:scale-95"><Download size={18} className="text-indigo-400" /><span className="hidden sm:inline">Backup JSON</span></button>
            <button onClick={exportToCSV} title="Exportar para Planilha" className="flex items-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 px-5 py-3 rounded-2xl font-bold text-sm transition-all active:scale-95"><FileSpreadsheet size={18} className="text-emerald-400" /><span className="hidden sm:inline">Exportar CSV</span></button>
          </div>
                    ...
                    <tr key={event.id} className="hover:bg-white/[0.02] transition-colors group">
                      ...
                      <td className="px-8 py-6">
                        <div className="flex justify-center gap-3">
                          ...
                          <button onClick={() => handlePrintClick(event)} className="p-3 bg-white/5 hover:bg-white/10 text-white rounded-xl transition-all active:scale-90"><Printer size={18} /></button>
                          <button onClick={(e) => handleDeleteClick(event.id, e)} className="p-3 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-xl transition-all active:scale-90"><Trash2 size={18} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
                  ...
                  <div className="bg-[#15162b] rounded-[3rem] p-10 border border-white/10 shadow-2xl relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                      ...
                      <div className="flex gap-4">
                        <button onClick={() => handlePrintClick(selectedEvent)} disabled={isProcessing} className={`flex-1 md:flex-none flex items-center justify-center gap-3 px-10 py-5 rounded-2xl font-black transition-all active:scale-95 ${isProcessing ? 'bg-slate-700 animate-pulse' : 'bg-white text-indigo-950 hover:bg-slate-200 shadow-xl'}`}>{isProcessing ? 'GERANDO PDF...' : <><Printer size={20} /> GERAR PDF</>}</button>
                        <button onClick={() => handleDeleteClick(selectedEvent.id)} className="p-5 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-2xl border border-red-500/10 transition-all active:scale-90"><Trash2 size={24} /></button>
                      </div>
                    </div>
                  </div>
      ...
      {isLayoutModalOpen && (
        <LayoutModal onSelect={handleLayoutSelect} onClose={() => { setIsLayoutModalOpen(false); setEventToPrint(null); }} />
      )}

      {eventToDelete && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/90 backdrop-blur-xl">
          <div className="w-full max-w-md bg-[#15162b] rounded-[2.5rem] border border-white/10 shadow-2xl p-8 text-center animate-in zoom-in duration-200">
            <div className="w-20 h-20 bg-red-500/20 rounded-3xl flex items-center justify-center mb-6 mx-auto">
              <Trash2 size={40} className="text-red-400" />
            </div>
            <h3 className="text-2xl font-black text-white mb-2">Excluir Sorteio?</h3>
            <p className="text-slate-400 mb-8 leading-relaxed">Esta ação é permanente e não pode ser desfeita. Deseja continuar?</p>
            <div className="flex gap-4">
              <button onClick={() => setEventToDelete(null)} className="flex-1 py-4 bg-white/5 hover:bg-white/10 text-slate-400 rounded-2xl font-black uppercase text-xs tracking-widest transition-all">Cancelar</button>
              <button onClick={confirmDelete} className="flex-1 py-4 bg-red-600 hover:bg-red-500 text-white rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl shadow-red-600/20 transition-all">Excluir</button>
            </div>
          </div>
        </div>
      )}
      ...
      {pendingRestoreData && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/90 backdrop-blur-xl">
          <div className="w-full max-w-md bg-[#15162b] rounded-[2.5rem] border border-white/10 shadow-2xl p-8 text-center animate-in zoom-in duration-200">
            <div className="w-20 h-20 bg-amber-500/20 rounded-3xl flex items-center justify-center mb-6 mx-auto">
              <Database size={40} className="text-amber-400" />
            </div>
            <h3 className="text-2xl font-black text-white mb-2">Restaurar Backup?</h3>
            <p className="text-slate-400 mb-8 leading-relaxed">Isso irá substituir <strong>todos</strong> os dados atuais pelos dados do arquivo de backup. Esta ação não pode ser desfeita.</p>
            <div className="flex gap-4">
              <button onClick={() => setPendingRestoreData(null)} className="flex-1 py-4 bg-white/5 hover:bg-white/10 text-slate-400 rounded-2xl font-black uppercase text-xs tracking-widest transition-all">Cancelar</button>
              <button onClick={confirmRestore} className="flex-1 py-4 bg-amber-600 hover:bg-amber-500 text-white rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl shadow-amber-600/20 transition-all">Restaurar</button>
            </div>
          </div>
        </div>
      )}
